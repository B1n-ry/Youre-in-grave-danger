...
public abstract class LivingEntityMixin {
    ...
    private void generateGrave(LivingEntity livingEntity, DamageSource source) {
        if (!(livingEntity instanceof PlayerEntity) || livingEntity.world.getGameRules().getBoolean(GameRules.KEEP_INVENTORY)) {
            this.dropInventory();
            return;
        }
        ...
        Yigd.NEXT_TICK.add(() -> {
            ...
            if (!config.generateGraves || config.blacklistDimensions.contains(dimId) || config.ignoreDeathTypes.contains(source.name) || !canGenerate) {
                ...
                GraveHelper.dropExp((ServerWorld) player.world, player.getPos(), xpPoints);
                return;
