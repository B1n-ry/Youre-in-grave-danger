...
public class GraveComponent {
    ...
    public DirectionalPos findGravePos(Direction defaultDirection) {
        ...
        if (config.graveConfig.tryGenerateOnGround) {
            for (BlockPos pos = this.pos.down(); pos.getY() >= this.world.getBottomY(); pos = pos.down()) {
                if (!this.world.getBlockState(pos).isIn(YigdTags.REPLACE_SOFT_WHITELIST)) {
                    this.pos = pos.up();
                    break;
                }
            }
    ...
    private DirectionalPos findPosInGraveyard(Direction defaultDirection) {
        ...
        for (GraveyardData.GraveLocation location : graveyardData.graveLocations) {
            ...
            if (!graveyardWorld.getBlockState(graveyardPos).isIn(YigdTags.REPLACE_SOFT_WHITELIST) || graveyardWorld.getBlockEntity(graveyardPos) != null)
                continue;
    ...
    public void placeAndLoad(Direction direction, DeathContext context, BlockPos pos, ServerWorld level, RespawnComponent respawnComponent) {
        YigdConfig config = YigdConfig.getConfig();

        ServerWorld deathWorld = context.world();
        ...
        boolean waterlogged = level.getFluidState(pos).isOf(Fluids.WATER);  // Grave generated in full water block (submerged)
        ...
        Yigd.END_OF_TICK.add(() -> {
            ...
            if (!placed) {
                Yigd.LOGGER.error("Failed to generate grave at X: {}, Y: {}, Z: {}, {}. Grave block placement failed",
                        placedPos.getX(), placedPos.getY(), placedPos.getZ(), level.getRegistryKey().getValue());
                ...
                this.getInventoryComponent().dropGraveItems(level, Vec3d.of(placedPos));
                this.getExpComponent().dropAll(level, Vec3d.of(placedPos));
                return;
            }
    ...
    public boolean replaceWithOld(BlockState newState, boolean safetyOn) {
        ...
        if (safetyOn && newState.isIn(YigdTags.REPLACE_GRAVE_BLACKLIST)) return false;
    ...
    public void dropAllGraveItems() {
        this.inventoryComponent.filteredInv(rule -> rule == DropRule.PUT_IN_GRAVE)
                .dropAll(this.world, this.pos.toCenterPos());
