...
public class TravelersBackpackCompat implements YigdApi {
    ...
    public Object getInventory(PlayerEntity player, boolean onDeath) {
        ...
        if (onDeath) {
            ...
            if (GraveHelper.hasEnchantments(soulboundEnchantments, backpack) || player.world.getTagManager().getOrCreateTagGroup(Registry.ITEM_KEY).getTagsFor(backpack.getItem()).contains((new Identifier("yigd", "soulbound_item")))) {
                soulbound = backpack;
                shouldDelete = true;
            } else {
