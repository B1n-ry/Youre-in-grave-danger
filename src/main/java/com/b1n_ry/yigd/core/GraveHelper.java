...
public class GraveHelper {
    ...
    public static void dropExp(ServerWorld world, Vec3d pos, int amount) {
        while(amount > 0) {
    ...
    public static void placeDeathGrave(World world, Vec3d pos, PlayerEntity player, DefaultedList<ItemStack> invItems, List<Object> modInventories, int xpPoints, DamageSource source) {
            ...
            if (json instanceof JsonArray) {
                ...
                if (!point2point) {
                    for (JsonElement blockPosition : coordinates) {
                        if (blockPosition instanceof JsonObject) {
                            JsonObject xyz = (JsonObject) blockPosition;
                    ...
                    if (coordinates.get(0) instanceof JsonObject && coordinates.get(1) instanceof JsonObject) {
                        JsonObject pos1 = (JsonObject) coordinates.get(0);
                        JsonObject pos2 = (JsonObject) coordinates.get(1);
        ...
        if (!foundViableGrave) { // No grave was placed
            ...
            if (!isPlaced) {
                ...
                dropExp((ServerWorld) world, new Vec3d(blockPos.getX(), blockPos.getY(), blockPos.getZ()), xpPoints);
    ...
    private static boolean gravePlaceableAt(World world, BlockPos blockPos, boolean strict) {
        ...
        return !(xPos >= boundEast && xPos <= boundWest && yPos >= 0 && yPos <= 256 && zPos <= boundNorth && zPos >= boundSouth);
    }
