...
public class GraveHelper {
    ...
    public static void deleteItemFromList(DefaultedList<ItemStack> itemList, boolean asStack, Predicate<ItemStack> canDeleteItem) {
        ...
        for (int i = 0; i < itemList.size(); i++) {
            ...
            if (!canDeleteItem.test(stack)) continue;
    ...
    public static void placeDeathGrave(World world, Vec3d pos, PlayerEntity player, DefaultedList<ItemStack> invItems, List<Object> modInventories, int xpPoints, DamageSource source) {
        ...
        if (!foundViableGrave) { // No grave was placed
            ...
            if (!isPlaced) {
                ...
                for (YigdApi yigdApi : Yigd.apiMods) {
                    int index = Yigd.apiMods.indexOf(yigdApi);
                    Object o = modInventories.get(index);
