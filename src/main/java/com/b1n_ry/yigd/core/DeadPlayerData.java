...
public class DeadPlayerData {
    ...
    public static DeadPlayerData create(DefaultedList<ItemStack> inventory, List<Object> modInventories, BlockPos gravePos, GameProfile graveOwner , int xp, World world, DamageSource deathSource) {
        ...
        return new DeadPlayerData(inventory, modInventories, gravePos, graveOwner, xp, world.getRegistryKey().getValue(), dimName, deathSource);
    }
    public DeadPlayerData(DefaultedList<ItemStack> inventory, List<Object> modInventories, BlockPos gravePos, GameProfile graveOwner, int xp, Identifier worldId, String dimensionName, DamageSource deathSource) {
    ...
    public static DeadPlayerData fromNbt(NbtCompound nbt, @Nullable ServerWorld world) {
        ...
        if (itemNbt instanceof NbtCompound) {
            NbtCompound itemNbtCompound = (NbtCompound) itemNbt;
            Inventories.readNbt(itemNbtCompound, items);
        }
        NbtElement modNbt = nbt.get("modInventory");
        List<Object> modInventories = new ArrayList<>();
        if (modNbt instanceof NbtCompound) {
            NbtCompound modNbtCompound = (NbtCompound) modNbt;
        ...
        if (blockPosNbt instanceof NbtCompound) {
            NbtCompound c = (NbtCompound) blockPosNbt;
        ...
        if (ownerElement instanceof NbtCompound) {
            NbtCompound ownerNbt = (NbtCompound) ownerElement;
        ...
        if (damageSourceNbt instanceof NbtCompound) {
            NbtCompound dsNbt = (NbtCompound) damageSourceNbt;
            ...
            if (source != null) {
                ...
                switch (sourceName) {
                    case "inFire": deathSource = DamageSource.IN_FIRE; break;
                    case "lightningBolt": deathSource = DamageSource.LIGHTNING_BOLT; break;
                    case "onFire": deathSource = DamageSource.ON_FIRE; break;
                    case "lava": deathSource = DamageSource.LAVA; break;
                    case "hotFloor": deathSource = DamageSource.HOT_FLOOR; break;
                    case "inWall": deathSource = DamageSource.IN_WALL; break;
                    case "cramming": deathSource = DamageSource.CRAMMING; break;
                    case "drown": deathSource = DamageSource.DROWN; break;
                    case "starve": deathSource = DamageSource.STARVE; break;
                    case "cactus": deathSource = DamageSource.CACTUS; break;
                    case "fall": deathSource = DamageSource.FALL; break;
                    case "flyIntoWall": deathSource = DamageSource.FLY_INTO_WALL; break;
                    case "outOfWorld": deathSource = DamageSource.OUT_OF_WORLD; break;
                    case "magic": deathSource = DamageSource.MAGIC; break;
                    case "wither": deathSource = DamageSource.WITHER; break;
                    case "anvil": deathSource = DamageSource.ANVIL; break;
                    case "fallingBlock": deathSource = DamageSource.FALLING_BLOCK; break;
                    case "dragonBreath": deathSource = DamageSource.DRAGON_BREATH; break;
                    case "dryout": deathSource = DamageSource.DRYOUT; break;
                    case "sweetBerryBush": deathSource = DamageSource.SWEET_BERRY_BUSH; break;
        ...
        return new DeadPlayerData(items, modInventories, pos, graveOwner, xp, worldIdentifier, dimName, deathSource);
    }
    ...
    public static class Soulbound {
        ...
        public static void fromNbt(NbtCompound nbt) {
            ...
            NbtList vanillaList = nbt.getList("vanilla", NbtType.COMPOUND);
            NbtList modList = nbt.getList("mods", NbtType.COMPOUND);

            for (NbtElement e : vanillaList) {
                if (!(e instanceof NbtCompound)) continue;
                NbtCompound cVanilla = (NbtCompound) e;
            ...
            for (NbtElement e : modList) {
                if (!(e instanceof NbtCompound)) continue;
                NbtCompound cMods = (NbtCompound) e;
